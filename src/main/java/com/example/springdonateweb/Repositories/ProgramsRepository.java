...
public interface ProgramsRepository extends JpaRepository<ProgramsEntity, Integer> {
    ...
    @Query("SELECT p.name AS name, p.goalAmount AS goalAmount, p.currentAmount AS currentAmount " +
            "FROM ProgramsEntity p " +
            "WHERE p.status = true " +
            "ORDER BY (p.currentAmount / p.goalAmount) DESC")
    List<Map<String, Object>> getProgramProgress();
